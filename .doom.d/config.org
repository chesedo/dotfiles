...
(setq doom-theme 'doom-alpine-dusk)
...
(after! lsp-rust
  (setq lsp-rust-analyzer-server-display-inlay-hints t
        ...
        lsp-rust-analyzer-cargo-target-dir t ;; Use a different target directory to prevent locks with cargo
...
#+begin_src emacs-lisp
(map! :map dape-breakpoint-mode-map
...
Configure =codelldb= path for Rust debugging with proper pretty-printers.

**** Why this is needed
On NixOS, packages are installed in the nix store (e.g., =/nix/store/...=), not in traditional locations. Dape's default configuration assumes =codelldb= is manually installed in =~/.config/doom/debug-adapters/=, which doesn't work with our declarative nix setup.

**** What it does
1. Searches system PATH for the =codelldb= binary (installed via =vscode-extensions.vadimcn.vscode-lldb.adapter=)
2. Updates the =codelldb-rust= configuration in =dape-configs= to use the system path
3. This allows Rust debugging with proper vector pretty-printing support

#+begin_src emacs-lisp
(after! dape
  (when-let ((codelldb-path (executable-find "codelldb")))
    (setf (alist-get 'codelldb-rust dape-configs)
          (plist-put (alist-get 'codelldb-rust dape-configs)
...
** Agentic
Testing config for the LLM integration
...
(after! gptel
  (setq
   gptel-model "docker.io/ai/qwen3-coder:64k"
   gptel-backend (gptel-make-openai "Docker Model Runner"
                   :host "wsl:12434"
                   :protocol "http"
                   :stream t
                   :key "ignored"
                   :models '("docker.io/ai/qwen3-coder:32k"
                             "docker.io/ai/qwen3-coder:48k"
                             "docker.io/ai/qwen3-coder:64k"
                             "docker.io/ai/qwen3-coder:30B-A3B-UD-Q4_K_XL"
                             "docker.io/ai/qwen2.5:3B-Q4_K_M"))
   gptel-default-mode 'org-mode
   gptel-prompt-prefix-alist '((org-mode . "* "))
   gptel-response-prefix-alist '((org-mode . "**Qwen:**\n\n"))))
#+end_src

* Project Notes
A capture-buffer workflow for per-file notes in =NOTES.md= at the project root.
Sections are kept sorted by file path. =C-c C-c= to commit, =C-c C-k= to abort.

#+begin_src emacs-lisp
(define-minor-mode chesedo-capture-mode
  "Active in project-note capture buffers."
  :keymap (make-sparse-keymap))

(map! :map chesedo-capture-mode-map
      :nvi "C-c C-c" #'chesedo--finalize-project-note
      :nvi "C-c C-k" #'chesedo--abort-project-note)

(defun chesedo--insert-sorted-section (header content)
  "Insert HEADER and CONTENT before the first ## heading that sorts after it."
  (goto-char (point-min))
  (let (inserted)
    (while (and (not inserted) (re-search-forward "^## " nil t))
      (let ((line-start (match-beginning 0))
            (line-text (buffer-substring-no-properties
                        (match-beginning 0) (line-end-position))))
        (when (string< header line-text)
...
(defun chesedo--finalize-project-note ()
  "Append the capture buffer content to NOTES.md and restore window layout."
  (interactive)
  (let ((content (string-trim (buffer-string)))
        (notes-file chesedo--capture-notes-file)
        (header chesedo--capture-header)
        (window-config chesedo--capture-window-config))
    (when (string-empty-p content)
      (user-error "Nothing to capture — use C-c C-k to abort"))
    (with-current-buffer (find-file-noselect notes-file)
      (goto-char (point-min))
      (if (and header (search-forward header nil t))
          (let ((end (save-excursion
            ...
            (skip-chars-backward "\n")
            (insert "\n" content "\n"))
        (chesedo--insert-sorted-section (or header "") content))
      (save-buffer))
    (kill-buffer)
    (set-window-configuration window-config)
    (message "Note captured to NOTES.md")))

(defun chesedo--abort-project-note ()
  "Discard the project note capture and restore window layout."
  (interactive)
  (let ((window-config chesedo--capture-window-config))
    (kill-buffer)
    (set-window-configuration window-config)
    (message "Capture aborted")))

(defun chesedo/capture-project-note ()
  "Open a capture buffer for a note about the current file in NOTES.md."
  (interactive)
  (unless (projectile-project-p)
    (user-error "Not in a project"))
  (when (get-buffer "*Project Note*")
    (user-error "Capture already in progress — finish it first (C-c C-c / C-c C-k)"))
  (let* ((project-root (projectile-project-root))
         (notes-file (expand-file-name "NOTES.md" project-root))
         (current-file (buffer-file-name))
         (relative-path (when current-file
                          (file-relative-name current-file project-root)))
         (header (when relative-path
                   (format "## [%s](%s)" relative-path relative-path)))
         (window-config (current-window-configuration))
         (capture-buffer (get-buffer-create "*Project Note*")))
    (with-current-buffer capture-buffer
      (erase-buffer)
      (markdown-mode)
      (chesedo-capture-mode 1)
      (setq-local chesedo--capture-notes-file notes-file)
      (setq-local chesedo--capture-header header)
      (setq-local chesedo--capture-window-config window-config))
    (select-window (split-window-below -15))
    (switch-to-buffer capture-buffer)
    (evil-insert-state)))

(map! :leader :desc "Capture project note" "n p" #'chesedo/capture-project-note)
#+end_src
